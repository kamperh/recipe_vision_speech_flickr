...
"""
Train a bag-of-words CNN on transcriptions, using a custom pooling layer.

In contrast to `train_bow_cnn.py` where only max pooling is performed, here
different types of pooling can be selected. Moreover, where the former performs
max pooling over the entire CNN output including regions corresponding to
padding (might not be zero because of bias terms), here pooling is only
performed over valid regions.
...
default_options_dict = {
    ...
    "model_dir": "models/train_bow_cnn_custompool",
    "n_padded": 800,
    "n_most_common": 1000,
    "n_max_epochs": 25,
    "batch_size": 128, # 30,
...
def build_bow_cnn_custompool_from_options_dict(x, x_lengths, keep_prob, options_dict):
...
def train_bow_cnn_custompool(options_dict=None, config=None, model_dir=None):
    ...
    print datetime.now()
    print "Building bag-of-words CNN with custom pooling"
    ...
    cnn = build_bow_cnn_custompool_from_options_dict(x, x_lengths, keep_prob, options_dict)
...
def main():
    ...
    options_dict["script"] = "train_bow_cnn_custompool"

    # Check model options
    assert options_dict["pool_shapes"][-1] is None

    # Train model
    train_bow_cnn_custompool(options_dict, config, model_dir=args.model_dir)
